...
function get_menu()
{
    ...
    //get menu
    $arr = @$CI->m_global->getDataAll('sys_role',NULL, ['is_active' => '1'], '"menu", "menu_json"', '"id" = '.$role)[0];
    $id_menu    = $arr->menu;
    ...
    $arr = @$CI->m_global->getDataAll('sys_menu', NULL, ['is_active' => '1'], 'id, name, icon, icon_img, folder, controler', $where_e, ['order', 'asc']);
    $menu = [];
    foreach($arr as $row){
        $menu[$row->id]['name']      = $row->name;
        $menu[$row->id]['icon']      = $row->icon;
        $menu[$row->id]['icon_img']  = $row->icon_img;
...
function get_menu_sub($row, $idmenu, $where_e)
{
    ...
    $tmp2 = @$CI->m_global->getDataAll('sys_menu', NULL, ['parent' => $idmenu, 'is_active' => '1'], 'id, name, icon, icon_img, folder, controler', $where_e, ['order', 'asc']);

    if(!empty($tmp2)){

        foreach($tmp2 as $row2){

            $tmp3 = $CI->m_global->getDataAll('sys_menu', NULL, ['is_active' => '1', 'parent' => $row2->id], 'id, name, icon, icon_img, folder, controler', $where_e, ['order', 'asc']);
