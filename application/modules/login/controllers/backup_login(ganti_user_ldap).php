...
class Login extends MX_Controller {
        ...
        if($login != ''){
            ...
            if($jum_role > 1){
                ...
                if($ROLE_SUB_ID <> '' && @$this->session->userdata('USER')['USER_ROLE_SUB'] == ''){
                    $data['arr_role_sub'] = $this->m_global->getDataAll('m_role_sub', NULL, "ROLE_ID = '$role_id'",'*');
                    $this->load->view('index', $data);
                }else{
    ...
    public function ldap_verification($username, $password, $ke='', $other_user='') {
        ...
        //jika user ldap tidak ada dan user login kembali
        if($ke == '2'){ $this->redirect_login(8);exit; }

        //cek username di ldap
