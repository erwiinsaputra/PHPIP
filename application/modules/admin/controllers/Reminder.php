...
class Reminder extends MX_Controller {
    ...
    private $prefix         = 'reminder';
    private $table_db       = 'sys_reminder';
    private $title          = 'Reminder';
    private $url            = 'admin/reminder';
    ...
    public function index()
    {
        ...
        $js['custom']       = ['table_reminder'];
        $this->template->display($this->url.'/v_'.$this->prefix, $data, $js);
    }

    public function table_reminder()
    {    
        // load model view 
        // $this->load->model('global/m_reminder','m_reminder');
        ...
        if(isset($_REQUEST['filter'])){
            foreach ($_REQUEST['filter'] as $row) {
                ...
                if(@$row['name'] != ''){
                    ...
                    $name = @$row['name'];
                    // $name = $this->m_reminder->select($row['name']); $name = str_replace(' AS '.$row['name'], ' ', $name );
        ...
        // $select = $this->m_reminder->select($select);
        ...
        foreach ($result as $rows) {
            ...
            $isi['reminder_name']       = $rows->reminder_name;
            $isi['reminder_date']       = date('d F Y',strtotime($rows->reminder_date)).'<br>'.date('H:i',strtotime($rows->reminder_date));
    ...
    public function load_add() {
        csrf_init();
        $data['url'] = $this->url;
        $this->template->display_ajax($this->url.'/v_reminder_add', $data);
    }
    ...
    public function load_edit() {
        ...
        $data['data'] = $this->m_global->getDataAll('sys_reminder', null, ['id'=>$id], '*')[0];
        $this->template->display_ajax($this->url.'/v_reminder_edit', $data);
    }
    ...
    public function save_add() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            // Set Rule Login Form
            $this->form_validation->set_rules('reminder_name', 'Reminder Name', 'trim|xss_clean|required');
            ...
            if ($this->form_validation->run($this)) {
                ...
                $pecah = explode(':',@$this->input->post('reminder_time'));
                $time_new = str_pad($pecah[0],2,'0',STR_PAD_LEFT).':'.str_pad($pecah[1],2,'0',STR_PAD_LEFT).':00';

                $data['reminder_name']  = @$this->input->post('reminder_name');
                $data['reminder_date']  = @$this->input->post('reminder_date').' '.$time_new;
                $data['description']    = @$this->input->post('description');
                $data['created_date']   = date("Y-m-d H:i:s");
                $data['created_by']     = h_session('USERNAME');
                $result = $this->m_global->insert('sys_reminder', $data);
    ...
    public function save_edit() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            if ($this->form_validation->run($this)) {
                ...
                $result = $this->m_global->update('sys_reminder', $data, ['id' => $id]);
    ...
    public function delete_data() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            $data['is_active'] = $val;
            $res = $this->m_global->update('sys_reminder', $data, ['id' => $id]);
            if($val == 't'){
