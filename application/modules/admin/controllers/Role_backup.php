...
class Role extends MX_Controller {
    ...
    public function index()
    {
        ...
        $data['url']        = $this->url;
        $data['breadcrumb'] = ['Admin' => TRUE, $this->title => $this->url];
        $js['custom']       = ['table_role'];
        $this->template->display('admin/'.$this->prefix.'/v_'.$this->prefix, $data, $js);
    }
    ...
    public function get_table()
    {
        if(@$_REQUEST['customActionType'] == 'group_action'){
            ...
            if(in_array(@$_REQUEST['customActionName'], $aChk)){
                $this->change_status($_REQUEST['customActionName'], ['ROLE_ID IN ' => "(".implode(",", $_REQUEST['id']).")"], count($_REQUEST['id']) );
            }
        }
        ...
        foreach ($result as $rows) {
            $records["data"][] = [
                ...
                $this->_records_action($rows->ROLE_ID, $rows->ROLE_STATUS)
            ];
            $i++;
        }
    ...
    public function show_add()
    {
        ...
        $this->template->display('admin/'.$this->prefix.'/v_'.$this->prefix.'_add', $data, $js);
    }
    ...
    public function show_edit($id)
    {
        ...
        $record             = $this->m_global->getDataAll($this->table_db, NULL, [$this->table_db.'.ROLE_ID' => $id], 'ROLE_NAME, ROLE_MENU, ROLE_ID')[0];
        $data['data']       = $record;
        ...
        $this->template->display('admin/'.$this->prefix.'/v_'.$this->prefix.'_edit', $data, $js);
    }
    ...
    public function edit($id)
    {
        ...
        if (csrf_get_token() != $input['ex_csrf_token']){
            ...
            if ($this->form_validation->run($this))
            {
                ...
                $result = $this->m_global->update($this->table_db, $data, ['ROLE_ID' => $id]);
                if ($result)
                {
                    $res['status'] = 1;
    ...
    {
        if($stat){
            $result = $this->m_global->delete($this->table_db, ['ROLE_ID' => $id]);
            if($result){
                $data['status'] = 1;
            }else{
                $data['status'] = 0;
            ...
            $result = $this->m_global->update($this->table_db, ['ROLE_STATUS' => $status], ['ROLE_ID' => $id]);
            if($result){
                $data['status'] = 1;
            }else{
                $data['status'] = 0;
