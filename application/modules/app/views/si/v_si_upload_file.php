...
<form id="fileupload" action="<?=site_url($url);?>/upload_file_si" method="POST" enctype="multipart/form-data">

    <!-- HIDDEN FILE -->
	<input type="hidden" name="id" id="id_si" value="<?=@$id;?>"/>
...
<div class="table-container">
    <table class="table table-striped table-bordered table-hover">            
        ...
        <tbody class="list_file_upload_si" idnya="<?=@$id?>">
            <?=@$html_list_file_upload_si;?>
        </tbody>
    </table>
</div>
...
<script type="text/javascript">
    ...
    window.finish_upload = function(){
        window.list_file_upload_si();
        window.list_file_si();
    }

    window.list_file_upload_si = function(){
        var id          = "<?=@$id;?>";
        var param       = {id:id};
        var url         = "<?=site_url($url);?>/list_file_upload_si";
        Metronic.blockUI({ target: '.list_file_upload_si',  boxed: true});
        $.post(url, param, function(msg){
            $('.list_file_upload_si').html(msg);
            Metronic.unblockUI('.list_file_upload_si');
        });
    }
