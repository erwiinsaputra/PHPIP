        ...
        <div class="col-md-12" style="text-align:center;">
                ...
                <?php if(in_array( h_session('ROLE_ID'), h_role_admin())){ ?>
                    <?php if(@$status_year =='1' || @$status_year ==''){ ?>
                        <button year="<?=@$year?>" id_kpi_so="<?=@$data->id?>" title="Request Approval" val="2" class="btn btn_change_status" style="background:darkblue;color:white;">Request Approval</button>
                    <?php } ?>
                    <?php if(@$status_year =='2'){ ?>
                        <button year="<?=@$year?>" id_kpi_so="<?=@$data->id?>" title="Approve" val="3" class="btn btn-primary btn_change_status" style="background:darkblue;color:white;">Approve</button>
                        <button year="<?=@$year?>" id_kpi_so="<?=@$data->id?>" title="Reject" val="4" class="btn btn_change_status" style="cursor:pointer;color:#fff;background-color:red;">Reject</button>
                    <?php } ?>
                ...
                <?php if(h_session('ROLE_ID') == '8'){ ?>
                    <?php if(@$status_year =='2'){ ?>
                        <button year="<?=@$year?>" id_kpi_so="<?=@$data->id?>" title="Approve" val="3" class="btn btn-primary btn_change_status" style="background:darkblue;color:white;">Approve</button>
                        <button year="<?=@$year?>" id_kpi_so="<?=@$data->id?>" title="Reject" val="4" class="btn btn_change_status" style="cursor:text;color:#fff;background-color:red;">Reject</button>
                    <?php } ?>
                    <?php if(@$status_year =='3'){ ?>
                        <button class="btn btn-primary" style="cursor:text;color:#fff;background-color:#5cb85c;">Approved</button>
                    <?php } ?>
...
$(document).ready(function () {
    ...
    $('.table_year_<?=$year?>').on('click', '.btn_change_status', function(e) {
        var id_kpi_so = $(this).attr('id_kpi_so');
        ...
        function(){
                ...
                var param  = {id_kpi_so:id_kpi_so, year:year, val:val, token:token};
                $.post(url, param, function(msg){
