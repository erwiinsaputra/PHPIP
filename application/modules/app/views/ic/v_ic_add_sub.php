<form method="post" id="form_add_sub<?=$id?>" action="javascript:;" class="form-horizontal" enctype="multipart/form-data">
  <div class="form-body">
      <div class="row">
            <div class="col-md-12">
              ...
              <div class="form-group">
                  ...
                  <div class="col-md-8">
                      <input name="code" value="<?=$no_baru?>" type="text" class="form-control angka" placeholder="Number" data-bvalidator="required" autocomplete="off" >
                  </div>
              </div>
              <div class="form-group">
              ...
              <div class="form-group">
                  ...
                  <div class="col-md-8">
                        <input name="weighting_factor" value="" type="text" class="form-control cek_total_sub_weighting_factor" placeholder="Weighting Factor (%)" data-bvalidator="required" autocomplete="off">
            ...
            <div class="col-md-12" style="padding:0 45px 0 45px;">
                <style>
                    #load_table_sub_action_plan_add>tbody>tr>td{ padding: 1px; }
                </style>
                <table class="table table-bordered" id="load_table_sub_action_plan_add" style="width: 94%;">
                    <tbody>
  ...
  <div class="form-actions">
      <div class="row">
          <div class="col-md-12" style="text-align:center;">
                ...
                <button id="btn_save_sub" class="btn btn-primary">Save</button>
          </div>
      </div>
  </div>
</form>
...
$(document).ready(function () {
     ...
     //range date
     $("#form_add_sub<?=$id?> #start_date").datepicker({
        format: 'yyyy-mm-dd',
        viewMode: "days", 
        minViewMode: "days",
        autoclose: true,
        ...
        $("#form_add_sub<?=$id?> #end_date").datepicker('setStartDate', minDate);
        window.load_table_budget_ic();
    });
    $("#form_add_sub<?=$id?> #end_date").datepicker({
        format: 'yyyy-mm-dd',
        viewMode: "days", 
        minViewMode: "days",
        autoclose: true,
        ...
        var minDate = new Date(selected.date.valueOf());
        $("#form_add_sub<?=$id?> #start_date").datepicker('setEndDate', minDate);
        window.load_table_budget_ic();
    });
    ...
    //select2 biasa
    $("#form_add_sub<?=$id?> .select2_biasa").select2({
        minimumResultsForSearch: -1, minimumInputLength: -1, allowClear:true, dropdownAutoWidth : true,
    });


    //select pic multiple
    $("#form_add_sub<?=$id?> input[name='pic_action_plan']").select2({
        ...
        ajax: {
            ...
            data: function (term, page) {
                var pic_si = $('#form_add_sub<?=$id?> #pic_si').val();
    ...
    //save data
    $('#form_add_sub<?=$id?>').on('click','#btn_save_sub',function(){
        $('#form_add_sub<?=$id?>').bValidator();
        $('#form_add_sub<?=$id?>').submit();
        if($('#form_add_sub<?=$id?>').data('bValidator').isValid()){
            ...
            function(){
                    var url   = "<?=site_url($url);?>/save_add_sub";
                    var param = $('#form_add_sub<?=$id?>').serializeArray();
                    Metronic.blockUI({ target: '.modal-dialog',  boxed: true});
                    $.post(url,param,function(msg){
                        Metronic.unblockUI('.modal-dialog');
                        toastr.options = call_toastr('4000');
                        if(msg.status == '1'){
                            $('#popup_add_sub').modal('hide');
    ...
    $('#form_add_sub<?=$id?>').on('keyup', '.cek_total_sub_weighting_factor', function(e) {
        var val = $(this).val();
    ...
    window.cek_total_sub_weighting_factor = function(weighting){
        if(weighting != ''){
            ...
            if(cek != '-1'){
                ...
                $("#form_add_sub<?=$id?> .cek_total_sub_weighting_factor").val(c);
            }else{
                ...
                // var total_sub_weighting_all = parseFloat($("#form_add_sub<?=$id?> .total_sub_weighting_factor_all").val()).toFixed(2);
                ...
                // var total_sub_weighting_now = parseFloat($("#form_add_sub<?=$id?> .total_sub_weighting_factor_now").val()).toFixed(2);
                ...
                //     $("#form_add_sub<?=$id?> .cek_total_sub_weighting_factor").val(limit);
                // }else{
                    $("#form_add_sub<?=$id?> .cek_total_sub_weighting_factor").val(weighting);
                // }
            }
        }
    }
    window.cek_total_sub_weighting_factor('<?=($action_plan->weighting_factor - $total_weighting_factor_now)?>');
    //==============================================================================
