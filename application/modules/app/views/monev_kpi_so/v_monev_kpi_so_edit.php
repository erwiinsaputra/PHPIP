...
<form method="post" id="form_edit_<?=$id?>" action="javascript:;" class="form-horizontal" accept-charset="utf-8">
  <div class="form-body">
      <div class="row">
        <div class="col-md-12">
            ...
            <div class="form-group">
                <label class="control-label col-md-3"><b>Performance Analysis/<br>Recommendations</b>
                    <span class="required" aria-required="true"></span>
                </label>
                <div class="col-md-8">
                    <div style="text-align:center;font-weight:bold;">Keterangan :</div>
                    <textarea name="penyebab1" class="form-control" rows="3" data-bvalidator="" autocomplete="off"><?=@$data->penyebab1;?></textarea>
                </div>
            ...
            <?php //if(@$data->month == '2' || @$data->month == '3'){ ?>
            <div class="form-group">
                <label class="control-label col-md-3"><b>Prognosa</b>
                    <span class="required" aria-required="true"></span>
                </label>
                <div class="col-md-8">
                    <input value="<?=@$data->prognosa;?>" name="prognosa" id="prognosa" type="text" class="form-control angka" data-bvalidator=""  autocomplete="off" >
                    <input value="<?=@$data->prognosa_pencapaian.' %';?>" name="prognosa_pencapaian" id="prognosa_pencapaian" readonly="readonly" type="text" class="form-control" data-bvalidator=""  autocomplete="off" >
                </div>
            </div>
            <div class="form-group">
                <label class="control-label col-md-3"><b>Performance Analysis/<br>Quick Win</b>
                    <span class="required" aria-required="true"></span>
                </label>
                <div class="col-md-8">
                    <div style="text-align:center;font-weight:bold;">Keterangan :</div>
                    <textarea name="penyebab2" class="form-control" rows="3" data-bvalidator="" autocomplete="off"><?=@$data->penyebab2;?></textarea>
                </div>
                <div class="col-md-3"></div>
                <div class="col-md-8">
                    <div style="text-align:center;font-weight:bold;">Quick Win :</div>
                    <textarea name="quick_win" class="form-control" rows="3" data-bvalidator="" autocomplete="off"><?=@$data->recommendations;?></textarea>
                </div>
            </div>
            <?php //} ?>
      </div>
...
$(document).ready(function () {
    ...
    //change prognosa
    $('#form_edit_<?=$id?> #prognosa').on('keyup',function(){
        //prognosa
        var prognosa = $(this).val();
        var prognosa = prognosa.replace(/,/g , "");
        if(prognosa == ''){ 
            $('#form_edit_<?=$id?> #prognosa_pencapaian').val('0 %');
            return true;
        }
        var prognosa = parseFloat(prognosa).toFixed(10);

        //target year
        var target   = $('#form_edit_<?=$id?> #target_year').val();
        var target = target.replace(/,/g , "");
        if(target == ''){ target = '0';}

        //jika target 0
        var cek_target = $('#form_edit_<?=$id?> #target_year').val();
        if(cek_target == '0'){ 
            var target = '1'; 
            var prognosa = parseFloat(parseFloat(prognosa) + parseFloat(1)).toFixed(10); 
        }
        // alert(target+" "+prognosa);
       ...
       var polarisasi = $('#form_edit_<?=$id?> #polarisasi').val();
        ...
        if(isNaN(pencapaian)){pencapaian=0;}
        if(pencapaian != 0){ var pencapaian = pencapaian.replace(".00", ""); }
        $('#form_edit_<?=$id?> #prognosa_pencapaian').val(pencapaian+' %');
    });
