...
class Monev_sr extends MX_Controller {
    ...
    public function index($id_sr='')
    {
        ...
        $js['custom']       = ['table_monev_sr'];
        $this->template->display($this->url.'/v_'.$this->prefix, $data, $js);
    }

    public function table_monev_sr()
    {    
        // load model view 
        $this->load->model('app/m_monev_sr','m_monev_sr');
       ...
       //filtering global
        ...
        //tampilkan status yang sudah di approve
        $whereE .= " AND a.status_sr NOT IN(1,2) ";
        ...
        if(isset($_REQUEST['filter'])){
            foreach ($_REQUEST['filter'] as $row) {
                ...
                if(@$row['name'] != ''){
                    ...
                    // $name = @$row['name'];
                    $name = $this->m_monev_sr->select($row['name']); $name = str_replace(' AS '.$row['name'], ' ', $name );
        ...
        //select 
        //select 
        $select = [ 'id','code_monev_sr','name_monev_sr','name_strategic_theme','name_periode','name_bsc',
                        'pic_sr','name_pic_sr','status_sr','name_status_sr','name_polarisasi','description',
                        'is_active','created_date','created_by','updated_date','updated_by'
                    ];
        $select = array_unique(array_merge($select, $search));
        $select = $this->m_monev_sr->select($select);
    ...
    public function load_add() {
        ...
        $this->template->display_ajax($this->url.'/v_monev_sr_add', $data);
    ...
    public function load_table_month() {
        ...
        $select = $this->m_monev_sr->select_target_month($select);
        ...
        // echo '<pre>';print_r($recommendations);exit;

        $this->template->display_ajax($this->url.'/v_monev_sr_table_month', $data);
    }
    ...
    public function load_edit_month() {
        ...
        $this->template->display_ajax($this->url.'/v_monev_sr_edit', $data);
    }
