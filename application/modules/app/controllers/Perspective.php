...
class Perspective extends MX_Controller {
    ...
    private $prefix         = 'perspective';
    private $table_db       = 'm_perspective';
    private $title          = 'Perspective';
    private $url            = 'app/perspective';
    ...
    public function index()
    {
        ...
        $js['custom']       = ['table_perspective'];
        $this->template->display($this->url.'/v_'.$this->prefix, $data, $js);
    }

    public function table_perspective()
    {    
        // load model view 
        // $this->load->model('global/m_perspective','m_perspective');
        ...
        if(isset($_REQUEST['filter'])){
            foreach ($_REQUEST['filter'] as $row) {
                ...
                if(@$row['name'] != ''){
                    ...
                    $name = @$row['name'];
                    // $name = $this->m_perspective->select($row['name']); $name = str_replace(' AS '.$row['name'], ' ', $name );
        ...
        // $select = $this->m_perspective->select($select);
    ...
    public function load_add() {
        csrf_init();
        $data['url'] = $this->url;
        $this->template->display_ajax($this->url.'/v_perspective_add', $data);
    }
    ...
    public function load_edit() {
        ...
        $data['data'] = $this->m_global->getDataAll('m_perspective', null, ['id'=>$id], '*')[0];
        $this->template->display_ajax($this->url.'/v_perspective_edit', $data);
    }
    ...
    public function save_add() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            // Set Rule Login Form
            $this->form_validation->set_rules('name', 'Perspective', 'trim|xss_clean|required');
            ...
            if ($this->form_validation->run($this)) {
                ...
                $result = $this->m_global->insert('m_perspective', $data);
    ...
    public function save_edit() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            // Set Rule Login Form
            $this->form_validation->set_rules('name', 'Perspective', 'trim|required');
            $this->form_validation->set_rules('code', 'Code', 'trim|required');
            $this->form_validation->set_rules('description', 'Description', 'trim|required');
            if ($this->form_validation->run($this)) {
                ...
                $result = $this->m_global->update('m_perspective', $data, ['id' => $id]);
    ...
    public function delete_data() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            $data['is_active'] = $val;
            $res = $this->m_global->update('m_perspective', $data, ['id' => $id]);
            if($val == 't'){
