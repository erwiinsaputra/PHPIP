...
class Pic_ic extends MX_Controller {
    
    private $prefix         = 'pic_ic';
    private $table_db       = 'm_pic';
    private $title          = 'PIC IC';
    private $url            = 'app/pic_ic';
    ...
    public function index()
    {
        ...
        $js['custom']       = ['table_pic_ic'];
        $this->template->display($this->url.'/v_'.$this->prefix, $data, $js);
    }

    public function table_pic_ic()
    {    
        // load model view 
        // $this->load->model('global/m_pic','m_pic');
        ...
        if(isset($_REQUEST['filter'])){
            foreach ($_REQUEST['filter'] as $row) {
                ...
                if(@$row['name'] != ''){
                    ...
                    $name = @$row['name'];
                    // $name = $this->m_pic->select($row['name']); $name = str_replace(' AS '.$row['name'], ' ', $name );
        ...
        // $select = $this->m_pic->select($select);
        ...
        foreach ($result as $rows) {
            ...
            $isi['singkatan_posisi']    = $rows->singkatan_posisi;
            $isi['posisi']              = h_read_more($rows->posisi,20);
    ...
    public function load_add() {
        csrf_init();
        $data['url'] = $this->url;
        $this->template->display_ajax($this->url.'/v_pic_ic_add', $data);
    }
    ...
    public function load_edit() {
        ...
        $data['data'] = $this->m_global->getDataAll('m_pic', null, ['id'=>$id], '*')[0];
        $this->template->display_ajax($this->url.'/v_pic_ic_edit', $data);
    }
    ...
    public function save_add() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            // Set Rule Login Form
            $this->form_validation->set_rules('singkatan_posisi', 'Singkatan Posisi', 'trim|xss_clean|required');
            $this->form_validation->set_rules('posisi', 'Posisi', 'trim|required');
            if ($this->form_validation->run($this)) {

                //insert data
                $data['singkatan_posisi']   = @$this->input->post('singkatan_posisi');
                $data['posisi']             = @$this->input->post('posisi');
                $data['created_date']       = date("Y-m-d H:i:s");
                $data['created_by']         = h_session('USERNAME');
                $result = $this->m_global->insert('m_pic', $data);
    ...
    public function save_edit() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            if ($this->form_validation->run($this)) {
                ...
                $result = $this->m_global->update('m_pic', $data, ['id' => $id]);
    ...
    public function delete_data() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            $data['is_active'] = $val;
            $res = $this->m_global->update('m_pic', $data, ['id' => $id]);
            if($val == 't'){
