...
class Workunit extends MX_Controller {
    
    private $prefix         = 'workunit';
    private $table_db       = 'm_workunit';
    private $title          = 'Workunit';
    private $url            = 'app/workunit';
    ...
    public function index()
    {
        ...
        $js['custom']       = ['table_workunit'];
        $this->template->display($this->url.'/v_'.$this->prefix, $data, $js);
    }

    public function table_workunit()
    {    
        // load model view 
        // $this->load->model('global/m_workunit','m_workunit');
        ...
        if(isset($_REQUEST['filter'])){
            foreach ($_REQUEST['filter'] as $row) {
                ...
                if(@$row['name'] != ''){
                    ...
                    $name = @$row['name'];
                    // $name = $this->m_workunit->select($row['name']); $name = str_replace(' AS '.$row['name'], ' ', $name );
        ...
        // $select = $this->m_workunit->select($select);
        ...
        foreach ($result as $rows) {
            ...
            $isi['name']                = $rows->name;
            $isi['is_corporate']        = ($rows->is_corporate == 't' ? 'Corporate' : 'Non Corporate');
            $isi['code']                = $rows->code;
    ...
    public function load_add() {
        csrf_init();
        $data['url'] = $this->url;
        $this->template->display_ajax($this->url.'/v_workunit_add', $data);
    }
    ...
    public function load_edit() {
        ...
        $data['data'] = $this->m_global->getDataAll('m_workunit', null, ['id'=>$id], '*')[0];
        $this->template->display_ajax($this->url.'/v_workunit_edit', $data);
    }
    ...
    public function save_add() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            // Set Rule Login Form
            $this->form_validation->set_rules('name', 'workunit', 'trim|xss_clean|required');
            if ($this->form_validation->run($this)) {
                ...
                $result = $this->m_global->insert('m_workunit', $data);
    ...
    public function save_edit() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            // Set Rule Login Form
            $this->form_validation->set_rules('name', 'workunit', 'trim|required');
            ...
            if ($this->form_validation->run($this)) {
                ...
                $result = $this->m_global->update('m_workunit', $data, ['id' => $id]);
    ...
    public function delete_data() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            $data['is_active'] = $val;
            $res = $this->m_global->update('m_workunit', $data, ['id' => $id]);
            if($val == 't'){
