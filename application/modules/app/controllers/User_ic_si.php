...
class User_ic_si extends MX_Controller {
    
    private $prefix         = 'user_ic_si';
    private $table_db       = 'm_user_ic_si';
    private $title          = 'User IC & SI';
    private $url            = 'app/user_ic_si';
    ...
    public function index()
    {
        ...
        $js['custom']       = ['table_user_ic_si'];
        $this->template->display($this->url.'/v_'.$this->prefix, $data, $js);
    }

    public function table_user_ic_si()
    {    
        // load model view 
        $this->load->model('app/m_user_ic_si','m_user_ic_si');
        ...
        if(isset($_REQUEST['filter'])){
            foreach ($_REQUEST['filter'] as $row) {
                ...
                if(@$row['name'] != ''){
                    ...
                    $name = @$row['name'];
                    $name = $this->m_user_ic_si->select($row['name']); $name = str_replace(' AS '.$row['name'], ' ', $name );
        ...
        $select = $this->m_user_ic_si->select($select);
        ...
        foreach ($result as $rows) {
            ...
            //isi table disini
            $code_name_si = '';
            $pecah = explode(', ',$rows->code_si);
            $pecah2 = explode(', ',$rows->name_si);
            foreach($pecah as $key=>$val){
                $code_name_si .= h_read_more($pecah[$key].'&nbsp;'.$pecah2[$key],30);
            }
    ...
    public function load_add() {
        csrf_init();
        $data['url'] = $this->url;
        $this->template->display_ajax($this->url.'/v_user_ic_si_add', $data);
    }
    ...
    public function load_edit() {
        ...
        $data['id'] = $id;
        $data['data'] = $this->m_global->getDataAll('m_user_ic_si', null, ['id'=>$id], '*')[0];
        $this->template->display_ajax($this->url.'/v_user_ic_si_edit', $data);
    }
    ...
    public function save_add() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            $this->form_validation->set_rules('id_si', 'SI Title', 'trim|required');
            if ($this->form_validation->run($this)) {
                
                //insert data
                $data['nip']            = @$this->input->post('nip');
                $data['id_si']          = str_replace(',',', ',@$this->input->post('id_si'));
    ...
    public function save_edit() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            if ($this->form_validation->run($this)) {
                ...
                $result = $this->m_global->update('m_user_ic_si', $data, ['id' => $id]);
    ...
    public function delete_data() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            $data['is_active'] = $val;
            $res = $this->m_global->update('m_user_ic_si', $data, ['id' => $id]);
            if($val == 't'){
    ...
    public function select_si()
    {
        ...
        if(isset($_REQUEST['q'])){
            ...
            $select     = 'id, name, code';
            $parent     = $this->m_global->getDataAll('m_si', NULL, $where, $select, NULL, 'code ASC');
            $data       = [];
    ...
    public function select_nip()
    {
        ...
        if(isset($_REQUEST['q'])){
            ...
            $where      .= " AND ( '5' = ANY (string_to_array(a.role_id,', ')) 
                                    OR '9' = ANY (string_to_array(a.role_id,', '))
                                )";
    ...
    public function cek_user_ic_si()
    {
        $where = " nip = '".$_POST['nip']."'";
        $cek   = @$this->m_global->getDataAll('m_user_ic_si', NULL, $where, 'id')[0]->id;
