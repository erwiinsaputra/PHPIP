...
class Periode extends MX_Controller {
    ...
    private $prefix         = 'periode';
    private $table_db       = 'm_periode';
    private $title          = 'Periode';
    private $url            = 'app/periode';
    ...
    public function index()
    {
        ...
        $js['custom']       = ['table_periode'];
        $this->template->display($this->url.'/v_'.$this->prefix, $data, $js);
    }

    public function table_periode()
    {    
        // load model view 
        $this->load->model('app/m_periode','m_periode');
        ...
        if(isset($_REQUEST['filter'])){
            foreach ($_REQUEST['filter'] as $row) {
                ...
                if(@$row['name'] != ''){
                    ...
                    $name = @$row['name'];
                    $name = $this->m_periode->select($row['name']); $name = str_replace(' AS '.$row['name'], ' ', $name );
        ...
        $select = $this->m_periode->select($select);
        ...
        foreach ($result as $rows) {
            ...
            $isi['start_year']          = $rows->start_year;
            $isi['end_year']            = $rows->end_year;
            $isi['name_strategic_theme'] = '- '.str_replace(', ','<br>- ',$rows->name_strategic_theme);
    ...
    public function load_add() {
        csrf_init();
        $data['url'] = $this->url;
        $this->template->display_ajax($this->url.'/v_periode_add', $data);
    }
    ...
    public function load_edit() {
        ...
        $data['data'] = $this->m_global->getDataAll('m_periode', null, ['id'=>$id], '*')[0];
        $this->template->display_ajax($this->url.'/v_periode_edit', $data);
    }
    ...
    public function save_add() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            // Set Rule Login Form
            $this->form_validation->set_rules('start_year', 'Start Year', 'trim|xss_clean|required');
            $this->form_validation->set_rules('end_year', 'End Year', 'trim|xss_clean|required');
            $this->form_validation->set_rules('id_strategic_theme', 'Strategi Theme', 'trim|xss_clean|required');
            if ($this->form_validation->run($this)) {

                //insert data
                $data['name']                   = @$this->input->post('start_year').' - '.@$this->input->post('end_year');
                $data['start_year']             = @$this->input->post('start_year');
                $data['end_year']               = @$this->input->post('end_year');
                $data['id_strategic_theme']     = str_replace(',',', ',@$this->input->post('id_strategic_theme'));

                $data['created_date']           = date("Y-m-d H:i:s");
                $data['created_by']             = h_session('USERNAME');
                $result = $this->m_global->insert('m_periode', $data);
                // echo '<pre>';print_r($data);exit;
                // echo $this->db->last_query();exit;

                //insert ke template strategy map
                $id_periode = $this->db->insert_id();
                $bsc = $this->m_global->getDataAll('m_bsc', null,  ['is_active'=>'t'], 'id',null, "id ASC");
                foreach($bsc as $row){
                    $data = [];
    ...
    public function save_edit() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            if ($this->form_validation->run($this)) {
                ...
                $result = $this->m_global->update('m_periode', $data, ['id' => $id]);
    ...
    public function delete_data() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            $data['is_active'] = $val;
            $res = $this->m_global->update('m_periode', $data, ['id' => $id]);
            $res = $this->m_global->update('m_template_strategy_map', $data, ['id_periode' => $id]);
            if($val == 't'){
    ...
    public function select_strategic_theme($id=NULL)
    {
        if(is_null($id)){
            $q          = $_GET['q'];
            $parent     = $this->m_global->getDataAll('m_strategic_theme', NULL,['name LIKE' => '%'.$q.'%'],'id, name',NULL,NULL,0,10);
            $data       = [];
            ...
            $id         = str_replace('-', ',', $id);
            $where_e    = " id IN (".$id.")";
            $parent     = $this->m_global->getDataAll('m_strategic_theme', NULL, NULL, '*', $where_e);
            $data       = [];
            for ($i=0; $i < count($parent); $i++) {
                $data[$i] = ['id' => $parent[$i]->id, 'name' => $parent[$i]->name];
            }
