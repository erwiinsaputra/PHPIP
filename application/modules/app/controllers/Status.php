...
class Status extends MX_Controller {
    ...
    private $prefix         = 'status';
    private $table_db       = 'm_status';
    private $title          = 'Status';
    private $url            = 'app/status';
    ...
    {    
        // load model view 
        // $this->load->model('global/m_status','m_status');
        ...
        if(isset($_REQUEST['filter'])){
            foreach ($_REQUEST['filter'] as $row) {
                ...
                if(@$row['name'] != ''){
                    ...
                    $name = @$row['name'];
                    // $name = $this->m_status->select($row['name']); $name = str_replace(' AS '.$row['name'], ' ', $name );
        ...
        // $select = $this->m_status->select($select);
    ...
    public function load_add() {
        csrf_init();
        $data['url'] = $this->url;
        $this->template->display_ajax($this->url.'/v_status_add', $data);
    }
    ...
    public function load_edit() {
        ...
        $data['data'] = $this->m_global->getDataAll('m_status', null, ['id'=>$id], '*')[0];
        $this->template->display_ajax($this->url.'/v_status_edit', $data);
    }
    ...
    public function save_add() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            // Set Rule Login Form
            $this->form_validation->set_rules('name', 'Status', 'trim|xss_clean|required');
            ...
            if ($this->form_validation->run($this)) {
                ...
                $result = $this->m_global->insert('m_status', $data);
    ...
    public function save_edit() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            if ($this->form_validation->run($this)) {
                ...
                $result = $this->m_global->update('m_status', $data, ['id' => $id]);
    ...
    public function delete_data() {
        ...
        if (csrf_get_token() != $ex_csrf_token){
            ...
            $data['is_active'] = $val;
            $res = $this->m_global->update('m_status', $data, ['id' => $id]);
            if($val == 't'){
