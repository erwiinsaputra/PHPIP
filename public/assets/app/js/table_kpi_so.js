var table_kpi_so = function () {
    return {
        ...
        initDefault: function (url, header, order, sort) {
            ...
            grid.init({
                ...
                dataTable: {
                    ...
                    "fnStateSaveParams": function (oSettings, oData) {
                        ...
                        $('.table_kpi_so .form-filter').each(function(index, el) {
                        ...
                        localStorage.setItem( 'DataTables_table_kpi_so_'+window.location.pathname, JSON.stringify(oData) );
                    },

                    //load parameter dari localstorage
                    "fnStateLoadParams": function(){
                        var filter = false;
                        var data = JSON.parse( localStorage.getItem('DataTables_table_kpi_so_'+window.location.pathname) );

                        $('.table_kpi_so .form-filter').each(function(){
                        ...
                        $('.table_kpi_so .global-filter').each(function(){
                            var name = $(this).attr('name');
                            if(data[name] !== undefined){
                                if($(this).val() == ''){
                                     $(this).val(data[name]);
            ...
            //untuk reload table
            gridT = grid;
            grid_table_kpi_so = grid;
            
            window.reload_table_kpi_so = function (){
                grid_table_kpi_so.getDataTable().ajax.reload(null, false);
            }
        }
    };
}();