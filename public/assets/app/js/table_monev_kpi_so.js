var table_monev_kpi_so = function () {
    return {
        ...
        initDefault: function (url, header, order, sort) {
            ...
            grid.init({
                ...
                dataTable: {
                    ...
                    "fnStateSaveParams": function (oSettings, oData) {
                        ...
                        localStorage.setItem( 'DataTables_table_monev_kpi_so_'+window.location.pathname, JSON.stringify(oData) );
                    },

                    //load parameter dari localstorage
                    "fnStateLoadParams": function(){
                        var filter = false;
                        var data = JSON.parse( localStorage.getItem('DataTables_table_monev_kpi_so_'+window.location.pathname) );

                        $('.table_monev_kpi_so .form-filter').each(function(){
                        ...
                        $('.table_monev_kpi_so .global-filter').each(function(){
                            var name = $(this).attr('name');
                            if(data[name] !== undefined){
                                if($(this).val() == ''){
                                     $(this).val(data[name]);
                        ...
                        if(filter){
                            $('.table_monev_kpi_so .filter').show();
                            // window.reload_table_monev_kpi_so();
                        }
                    },
                    "order": order
                }
            ...
            //untuk reload table
            gridT = grid;
            grid_table_monev_kpi_so = grid;
            
            window.reload_table_monev_kpi_so = function (){
                grid_table_monev_kpi_so.getDataTable().ajax.reload(null, false);
            }
        }
    };
}();