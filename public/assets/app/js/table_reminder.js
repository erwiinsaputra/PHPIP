    ...
    return {
        ...
        initDefault: function (url, header, order, sort) {
            ...
            grid.init({
                ...
                dataTable: {
                    ...
                    "fnStateSaveParams": function (oSettings, oData) {
                        ...
                        $('.table_reminder .form-filter').each(function(index, el) {
                        ...
                        $('.table_reminder .global-filter').each(function(index, el) {
                            var name = $(el).attr('name');
                            var val = $(el).val();
                            if($(this).val() != ''){
                                if(val != null){
                        ...
                        localStorage.setItem( 'DataTables_table_reminder_'+window.location.pathname, JSON.stringify(oData) );
                    },

                    //load parameter dari localstorage
                    "fnStateLoadParams": function(){
                        var filter = false;
                        var data = JSON.parse( localStorage.getItem('DataTables_table_reminder_'+window.location.pathname) );

                        $('.table_reminder .form-filter').each(function(){
                        ...
                        $('.table_reminder .global-filter').each(function(){
                            var name = $(this).attr('name');
                            if(data[name] !== undefined){
                                if($(this).val() == ''){
                                     $(this).val(data[name]);
            ...
            grid.getTableWrapper().on('keyup', '.form-filter', function (e) {
                if(e.keyCode == 13){
                    $('.table_reminder .filter-submit').first().click();
                }
            });
            ...
            //untuk reload table
            gridT = grid;
            grid_table_reminder = grid;
            
            window.reload_table_reminder = function (){
                grid_table_reminder.getDataTable().ajax.reload(null, false);
            }
        }
    };
}();