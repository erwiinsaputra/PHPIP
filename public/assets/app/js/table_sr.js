    ...
    return {
        ...
        initDefault: function (url, header, order, sort) {
            ...
            grid.init({
                ...
                dataTable: {
                    ...
                    "fnStateSaveParams": function (oSettings, oData) {
                        ...
                        $('.table_sr .form-filter').each(function(index, el) {
                        ...
                        $('.table_sr .global-filter').each(function(index, el) {
                            var name = $(el).attr('name');
                            var val = $(el).val();
                            if($(this).val() != ''){
                                if(val != null){
                        ...
                        localStorage.setItem( 'DataTables_table_sr_'+window.location.pathname, JSON.stringify(oData) );
                    },

                    //load parameter dari localstorage
                    "fnStateLoadParams": function(){
                        var filter = false;
                        var data = JSON.parse( localStorage.getItem('DataTables_table_sr_'+window.location.pathname) );

                        $('.table_sr .form-filter').each(function(){
                        ...
                        $('.table_sr .global-filter').each(function(){
                            var name = $(this).attr('name');
                            if(data[name] !== undefined){
                                if($(this).val() == ''){
                                     $(this).val(data[name]);
                        ...
                        if(filter){
                            $('.table_sr .filter').show();
                            // window.reload_table_sr();
                        }
                    },
                    "order": order
                }
            ...
            grid.getTableWrapper().on('keyup', '.form-filter', function (e) {
                if(e.keyCode == 13){
                    $('.table_sr .filter-submit').first().click();
                }
            });
            ...
            window.reload_table_sr = function (){
                grid_table_sr.getDataTable().ajax.reload(null, false);
            }
        }
    };
}();