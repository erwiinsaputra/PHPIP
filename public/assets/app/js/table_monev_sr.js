var table_monev_sr = function () {
    return {
        ...
        initDefault: function (url, header, order, sort) {
            ...
            grid.init({
                ...
                dataTable: {
                    ...
                    "fnStateSaveParams": function (oSettings, oData) {
                        ...
                        $('.table_monev_sr .form-filter').each(function(index, el) {
                        ...
                        $('.table_monev_sr .global-filter').each(function(index, el) {
                            var name = $(el).attr('name');
                            var val = $(el).val();
                            if($(this).val() != ''){
                                if(val != null){
                        ...
                        localStorage.setItem( 'DataTables_table_monev_sr_'+window.location.pathname, JSON.stringify(oData) );
                    },

                    //load parameter dari localstorage
                    "fnStateLoadParams": function(){
                        var filter = false;
                        var data = JSON.parse( localStorage.getItem('DataTables_table_monev_sr_'+window.location.pathname) );

                        $('.table_monev_sr .form-filter').each(function(){
                        ...
                        $('.table_monev_sr .global-filter').each(function(){
                            var name = $(this).attr('name');
                            if(data[name] !== undefined){
                                if($(this).val() == ''){
                                     $(this).val(data[name]);
            ...
            //untuk reload table
            gridT = grid;
            grid_table_monev_sr = grid;
            
            window.reload_table_monev_sr = function (){
                grid_table_monev_sr.getDataTable().ajax.reload(null, false);
            }
        }
    };
}();