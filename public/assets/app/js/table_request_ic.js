var table_request_ic = function () {
    return {
        ...
        initDefault: function (url, header, order, sort) {
            ...
            grid.init({
                ...
                dataTable: {
                    ...
                    "fnStateSaveParams": function (oSettings, oData) {
                        ...
                        $('.table_request_ic .form-filter').each(function(index, el) {
                        ...
                        $('.table_request_ic .global-filter').each(function(index, el) {
                            var name = $(el).attr('name');
                            var val = $(el).val();
                            if($(this).val() != ''){
                                if(val != null){
                        ...
                        localStorage.setItem( 'DataTables_table_request_ic_'+window.location.pathname, JSON.stringify(oData) );
                    },

                    //load parameter dari localstorage
                    "fnStateLoadParams": function(){
                        var filter = false;
                        var data = JSON.parse( localStorage.getItem('DataTables_table_request_ic_'+window.location.pathname) );

                        $('.table_request_ic .form-filter').each(function(){
                        ...
                        // $('.table_request_ic .global-filter').each(function(){
                        //     var name = $(this).attr('name');
                        //     if(data[name] !== undefined){
                        //         if(data[name] != null){
            ...
            //untuk reload table
            gridT = grid;
            grid_table_request_ic = grid;
            
            window.reload_table_request_ic = function (){
                grid_table_request_ic.getDataTable().ajax.reload(null, false);
            }
        }
    };
}();