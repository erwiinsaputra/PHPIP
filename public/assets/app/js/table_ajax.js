    ...
    return {
        ...
        initDefault: function (url, header, order, sort) {
            ...
            grid.init({
                ...
                dataTable: {
                    ...
                    "fnStateSaveParams": function (oSettings, oData) {
                        ...
                        $('.table_ajax .form-filter').each(function(index, el) {
                        ...
                        $('.table_ajax .global-filter').each(function(index, el) {
                            var name = $(el).attr('name');
                            var val = $(el).val();
                            if($(this).val() != ''){
                                if(val != null){
                        ...
                        localStorage.setItem( 'DataTables_table_ajax_'+window.location.pathname, JSON.stringify(oData) );
                    },

                    //load parameter dari localstorage
                    "fnStateLoadParams": function(){
                        var filter = false;
                        var data = JSON.parse( localStorage.getItem('DataTables_table_ajax_'+window.location.pathname) );

                        $('.table_ajax .form-filter').each(function(){
                        ...
                        $('.table_ajax .global-filter').each(function(){
                            var name = $(this).attr('name');
                            if(data[name] !== undefined){
                                if($(this).val() == ''){
                                     $(this).val(data[name]);
                        ...
                        if(filter){
                            $('.table_ajax .filter').show();
                            // window.reload_table_ajax();
                        }
                    },
                    "order": order
                }
            ...
            grid.getTableWrapper().on('keyup', '.form-filter', function (e) {
                if(e.keyCode == 13){
                    $('.table_ajax .filter-submit').first().click();
                }
            });
            ...
            //untuk reload table
            gridT = grid;
            grid_table_ajax = grid;
            
            window.reload_table_ajax = function (){
                grid_table_ajax.getDataTable().ajax.reload(null, false);
            }
        }
    };
}();